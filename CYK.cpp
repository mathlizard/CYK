...
bool CYK(rules const &nonterminal_rules, rules const &terminal_rules, string const &s, 
	...
	for (auto const &v : terminal_rules)
		for (i = 0; i < n; ++i)
			if (string(1,s[i]) == v[1])
				M[v[0]](i, i) = true;
...
int main() {
	...
	if ((read_from_file("derivation_rules.txt", derivation_rules) && 
		...
		string starting_symbol { nonterminals_vec.front() };
		
		// convert the nonterminals into a set; implemented by a balanced binary search tree (R-B)
		set<string> nonterminals {	std::make_move_iterator(nonterminals_vec.begin()), 
		...
		for (auto const &line : derivation_rules) {		// parsing 'line' we build 'r'
			...
			decompose(r, nonterminals, rule_set);		// transform r into chomsky nf, put the resulting rules into rule_set
			r.clear();									// empty them
			rhs.clear();
			tokens_of_line.clear();
		}
		rules terminal_rules; // set<rule>
		rules nonterminal_rules; // set<rule>
		for (auto const &r : rule_set)
			if (r.size() == 2)							// if the rule is terminal, i.e. A -> a
				terminal_rules.insert(r); // logarithmic time
			else										// otherwise, i.e. when it's A -> B C
				nonterminal_rules.insert(r); // logarithmic time
